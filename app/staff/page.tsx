...
export default function Page() {
      ...
      if (eventName.startsWith("patient-")) {
        ...
        setPatients((prevPatients) => {
          ...
          if (existingIndex > -1) {
            ...
            // Automatically select the new live session
  ...
  return (
    <div className="w-full min-h-screen bg-[#0B0F19] text-[#F8FAFC] flex font-sans">
      {/* 1. Left Sidebar */}
      <aside className="w-64 border-r border-[#1E293B]/60 bg-[#111622] flex flex-col justify-between py-6 px-4 shrink-0">
        <div>
          {/* Top Brand */}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-left text-xs font-semibold transition-all duration-200 group ${
                    isActive
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 border-b border-[#1E293B]/60 bg-[#0F1420]">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-lg font-bold tracking-wide text-white">Live Patient Dashboard</h2>
              <p className="text-[10px] text-[#64748B] mt-0.5">Monitor client patient-registration form inputs in real-time</p>
            </div>
            
            {/* Live Indicator */}
            <div className="flex items-center gap-3 bg-[#0F172A] border border-[#1E293B]/40 px-3.5 py-1.5 rounded-xl shadow-inner">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-[#161D2B] border border-[#10B981]/20 rounded-xl p-4 flex flex-col justify-between h-20 shadow-sm">
              <span className="text-[20px] font-extrabold text-[#10B981] leading-none">{stats.activeSessions}</span>
              <span className="text-[10px] font-semibold text-[#64748B]">Active Sessions</span>
            </div>
            <div className="bg-[#161D2B] border border-[#EAB308]/20 rounded-xl p-4 flex flex-col justify-between h-20 shadow-sm">
              <span className="text-[20px] font-extrabold text-[#EAB308] leading-none">{stats.fillingIn}</span>
              <span className="text-[10px] font-semibold text-[#64748B]">Filling In</span>
            </div>
            <div className="bg-[#161D2B] border border-[#3B82F6]/20 rounded-xl p-4 flex flex-col justify-between h-20 shadow-sm">
              <span className="text-[20px] font-extrabold text-[#3B82F6] leading-none">{stats.submittedToday}</span>
              <span className="text-[10px] font-semibold text-[#64748B]">Submitted Today</span>
            </div>
            <div className="bg-[#161D2B] border border-[#EF4444]/20 rounded-xl p-4 flex flex-col justify-between h-20 shadow-sm">
              <span className="text-[20px] font-extrabold text-[#EF4444] leading-none">{stats.pendingReview}</span>
              <span className="text-[10px] font-semibold text-[#64748B]">Pending Review</span>
            </div>
          </div>
        </div>

        {/* 3. Main Live Data Container */}
        <div className="flex-1 flex min-h-0 bg-[#0B0F19]">
          {/* Left: Active Patients list */}
          <section className="w-80 border-r border-[#1E293B]/60 p-4 overflow-y-auto flex flex-col shrink-0">
              ...
              {patients.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left relative overflow-hidden bg-[#161D2B] border rounded-xl p-4 hover:bg-[#1E283A]/30 transition-all duration-200 flex flex-col justify-between ${
                      isSelected
              ...
              <div className="max-w-3xl flex flex-col gap-6">
                ...
                <div className="flex justify-between items-start border-b border-[#1E293B]/40 pb-4">
                  ...
                  {selectedPatient.status === "active" && (
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#EAB308]/15 text-[#EAB308] border border-[#EAB308]/30 text-xs font-semibold">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#EAB308] animate-pulse"></span>
                      Filling In
                    </div>
                  )}
                  {selectedPatient.status === "inactive" && (
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#475569]/15 text-[#64748B] border border-[#475569]/30 text-xs font-semibold">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#64748B]"></span>
                      Inactive
                    </div>
                  )}
                  {selectedPatient.status === "submitted" && (
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#10B981]/15 text-[#10B981] border border-[#10B981]/30 text-xs font-semibold">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h4 className="text-xs font-extrabold tracking-wider uppercase text-[#64748B]">
